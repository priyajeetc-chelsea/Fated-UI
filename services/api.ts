...
class ApiService {
  ...
  async fetchAllMatches(): Promise<
    import("@/types/api").ApiAllMatchesResponse
  > {
    try {
  ...
  async fetchPotentialMatches(): Promise<
    import("@/types/api").ApiPotentialMatchesResponse
  > {
    try {
  ...
  async fetchConfirmedMatches(): Promise<
    import("@/types/api").ApiConfirmedMatchesResponse
  > {
    try {
  ...
  private getFilteredMockMatches(tagIds: number[]) {
        ...
        opinions: [
          {
            ...
            answer:
              "It's about equity, not just equality. Real change means changing systems, not just headlines.",
            tag: {
              tagId: 1,
              tagValue: "Feminism",
            },
        ...
        opinions: [
          {
            ...
            answer:
              "It's a necessity, but access is still a luxury. We need more awareness and affordability.",
            tag: {
              tagId: 2,
              tagValue: "Mental Health",
            },
        ...
        opinions: [
          {
            takeId: 503,
            question:
              "Do you believe nationalism is hurting India's diversity?",
            answer:
              "Yes, because hyper-nationalism silences dissent and erodes cultural pluralism.",
            tag: {
              tagId: 3,
              tagValue: "Politics",
            },
        ...
        opinions: [
          {
            ...
            answer:
              "We need systemic change, not just individual action. Corporations must be held accountable.",
            tag: {
              tagId: 4,
    ...
    return allMatches.filter((match) =>
      match.opinions.some((opinion) => tagIds.includes(opinion.tag.tagId)),
    );
  }
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      if (apiResponse.code === 200 && apiResponse.model) {
        ...
        if (
          apiResponse.model.onboardingStep &&
          apiResponse.model.onboardingStep.step < 5
        ) {
          console.log(
            "🚧 User in onboarding state:",
            apiResponse.model.onboardingStep,
          );
        ...
        const convertedResponse: MatchResponse = {
          ...
          matches: apiResponse.model.matches
            ? apiResponse.model.matches.map((match: any) => ({
                userId: match.userId,
                ...
                opinions: match.opinions.map((opinion: any) => ({
                  ...
                  tag: {
                    tagId: this.getTagIdFromName(
                      opinion.tag,
                      apiResponse.model.tags?.all || [],
                    ),
        ...
        throw new Error(
          `API returned error: ${apiResponse.msg || "Unknown error"}`,
        );
      }
    } catch (error) {
  ...
  async fetchMatches(request: MatchRequest): Promise<MatchResponse> {
    try {
      ...
      if (response.onboardingStep && response.onboardingStep.step < 5) {
        console.log(
          "🚧 User is in onboarding state, step:",
  ...
  async submitLifestyleDetails(
    lifestyleData: any,
  ): Promise<import("@/types/onboarding").LifestyleResponse> {
    try {
  ...
  async submitTakes(
    takesData: import("@/types/onboarding").NewTakesFormData,
  ): Promise<any> {
    try {
    ...
    try {
      ...
      if (Platform.OS === "android") {
        ...
        if (![200, 201, 204].includes(uploadResult.status)) {
          ...
          throw new Error(
            `S3 upload failed! status: ${uploadResult.status} - ${uploadResult.body}`,
          );
        }
      ...
      if (![200, 201, 204].includes(uploadResponse.status)) {
        ...
        throw new Error(
          `S3 upload failed! status: ${uploadResponse.status} - ${errorText}`,
        );
      }
  ...
  async getCurrentUserProfile(): Promise<
    import("@/types/api").CurrentUserProfileResponse
  > {
    try {
